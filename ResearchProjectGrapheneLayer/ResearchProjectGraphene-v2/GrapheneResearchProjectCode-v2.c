...
#include <pthread.h>
#include<time.h>
#include "ResearchProjectCodeSubFunctionHeader-v2.h"
...
void *CHEBYJACOBIThreadKernel(void *arguments){
    ...
    * Begin Relaxation
    */
    double Omega,OmegaGraphene,Convergence,epsilon1,epsilon2,UpdateFunc,epsilon,epsilon0,e,gamma;
    OmegaGraphene = 1;

    e = -1.60217662*pow(10,-19);
    gamma = 0.539*(e*pow(10,-9));
    ...
    while(Convergence > args->Tolerance){
        ...
        for(k = 0;k <2;k++){
            for(i = ExceptionStartValue+k;i<=ExceptionEndValue;i=i+2){
                ...
                HalfRows = ((args->ROWS)/2)-1;
                if(i >= (int)HalfRows+1){
                    epsilon1 = 1;
                    epsilon2 = 1;
                }

                else if(i<=(int)HalfRows-1){
                    epsilon1 = 4.1;
                    epsilon2 = 4.1;
                }
                else{
                ...
                for(j = k;j<=args->COLS-1;j = j+2){
                    ...
                    if(i == (int)HalfRows){
                        UpdateFunc = Dim1GrapheneLayerUpdateFunc(args->ArrayOld[i+1][j],args->ArrayOld[i-1][j],args->HStep,epsilon2,epsilon1,epsilon0,gamma,e);
                        epsilon = (UpdateFunc) - args->ArrayOld[i][j];
                        args->ArrayNew[i][j] = args->ArrayOld[i][j] + (OmegaGraphene*epsilon);
                    }
                    else if(j == 0){
                        UpdateFunc = 0.25*(2*args->ArrayOld[i][j+1]+args->ArrayOld[i+1][j]+args->ArrayOld[i-1][j]);
                        epsilon = (UpdateFunc) - args->ArrayOld[i][j];
                        args->ArrayNew[i][j] = args->ArrayOld[i][j] + (OmegaGraphene*epsilon);
                    }
                    else if(j == args->COLS-1){
                        UpdateFunc = 0.25*(2*args->ArrayOld[i][j-1]+args->ArrayOld[i+1][j]+args->ArrayOld[i-1][j]);
                        epsilon = (UpdateFunc) - args->ArrayOld[i][j];
                        args->ArrayNew[i][j] = args->ArrayOld[i][j] + (OmegaGraphene*epsilon);
                    }
                    else{
                        //UpdateFunc = Dim1GeneralUpdateFunc(args->ArrayOld[i+1][j],args->ArrayOld[i-1][j],epsilon2,epsilon1);
                        UpdateFunc = Dim2GeneralUpdateFunc(args->ArrayOld[i+1][j],args->ArrayOld[i-1][j],args->ArrayOld[i][j-1],args->ArrayOld[i][j+1],j);
                        epsilon = (UpdateFunc) - args->ArrayOld[i][j];
...
int ChebyshevParallel(int ROWS,int COLS,double HEIGHT,double RADIUS,double TOLERANCE,int THREADNUM){
/*
...
*/  register int i,j;
    ...
    */
    ...
    /*
    * End Thread groundwork
    */

    /*
    *Set scheduling policy, doesn't work on windows, works on Linux
    */
    ...
    * Set Arguments
    */
    //Convergence List for Pthreads to calculate convergence together i.e can take the sum of ConvList
    // to get the total convergence(not normalised though)
    double *ConvList = (double*) malloc(THREADNUM*sizeof(double));
    //Define arguments of struct in order to pass to various functions
    ...
    * Create Threads
    */
    //Timer to check execution time without overhead
    clock_t begin = clock();
    for (i = 0; i < THREADNUM; i++) {
...
int main(){
    ...
    THREADNUM = 12;
    ChebyshevParallel(ROWS,COLS,HEIGHT,RADIUS,TOLERANCE,THREADNUM);
